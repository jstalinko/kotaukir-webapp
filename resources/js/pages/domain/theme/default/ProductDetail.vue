...
// Get CTA settings from site components or use defaults
const ctaSettings = computed(() => {
    const comps = props.site?.components?.['default'] || [];
    const cta = comps.find(c => c.type === 'Cta');
    return cta?.settings || {
        text: 'Tanya Lewat WhatsApp',
...
const images = computed(() => {
    const parsed = parseImages(props.product?.images);
    return parsed.length > 0 ? parsed : ['https://placehold.co/800x1000/451A03/white?text=No+Image'];
});

const whatsappLink = computed(() => {
    const productTitle = props.product?.title || 'produk Anda';
    const siteName = props.user?.name || '';
    const number = ctaSettings.value.whatsapp_number || '6281234567890';
    const customMessage = ctaSettings.value.message || '';
    const text = customMessage || `Halo ${siteName}, saya tertarik dengan produk ${productTitle} yang saya lihat di website.`;
            ...
            <nav class="flex mb-8 text-xs font-bold uppercase tracking-widest" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-2">
                    ...
                    <li class="flex items-center space-x-2">
                        ...
                        <Link :href="`/domain/${user?.name}/products`" class="text-gray-400 hover:text-[#451A03]">Produk
                        </Link>
                    </li>
                ...
                <div class="lg:col-span-5 space-y-10">
                    ...
                    <div class="space-y-4 pt-6">
                        <a :href="whatsappLink" target="_blank"
                            ...
                            {{ ctaSettings.text || 'Tanya Lewat WhatsApp' }}
                        </a>
                    ...
                    <div class="p-6 bg-[#451A03]/5 rounded-[32px] border border-[#451A03]/5">
                        <p class="text-[10px] text-[#451A03]/50 font-bold uppercase tracking-widest text-center">
                            Setiap pembelian produk kayu di {{ user?.name }} berkontribusi pada program penanaman
                            kembali
                            hutan jati.
                        </p>
                    </div>
